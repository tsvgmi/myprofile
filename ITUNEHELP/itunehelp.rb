...
# Wrapper so accessor would be more ruby like
  ...
  class ITuneFolder
    ...
    # Cleanup the artist/composer field with a sanitized list
    # Thesa are spelled differently so need to be sanitized for
    # iTune to group in the correct folder
    ...
    def track_run(instruction)
      ...
      self.each_track do |atrack, iname|
        ...
        when 'name.composer'
          ...
          atrack.updates(updset)
        # Swap the name and artist field
        ...
        when 'artist.name'
          ...
          next unless title
          atrack.updates(:name => _mixcase(title), :artist => artist)
        ...
        when 'name'
          title = atrack.name
          atrack.updates(:name => _mixcase(title))
        when 'name.group'
          title, group = atrack.name.split(/\s*[-\(\)]\s*/)
          next unless group
          atrack.updates(:name => _mixcase(title), :grouping => group)
        when 'name.group.artist'
          title, group, tmp, artist = atrack.name.split(/\s*[-\(\)]\s*/)
          next unless group
          atrack.updates(:name => _mixcase(title), :artist=>artist, :grouping => group)
        # Capitalize name
        when 'cap'
          updset = {}
  ...
  class ITuneHelper
    ...
    # Select all files in view duplicates and run through this filter
    # to rate all lower bit rate bps with 1 star
    def self.rate_dups
      options = getOption
      tracks  = ITuneFolder.new("select", options).get_tracks
      otrack  = nil
      tracks.each do |atrack|
        Plog.info atrack.name
        ...
        if otrack && (atrack.name_clean == otrack.name_clean) &&
          ...
          puts "Size:   %8d - %8d" % [otrack.size, atrack.size]
          puts "Time:   %8s - %8s" % [otrack.time, atrack.time]
          # Rate 20 (1 star) for the smaller size b/c we want to
          # keep the higher bit rate around
          if otrack.size > atrack.size
            atrack.updates(:rating => 20)
          else
            otrack.updates(:rating => 20)
          end
        end
        otrack = atrack
      end
    ...
    def self.clone_meta
      ...
      while tracks.size > 0
        ...
        if new_track.composer.strip.empty?
          cset[:composer] = old_track.composer
        end
